...
### 路径占用查询
![路径查询](docs/screenshots/path-query.png)
...
1. **平台支持**: 目前仅支持 **Windows 10 / Windows 11**。

2. **权限要求**: 某些系统进程需要管理员权限才能查询或结束。应用会自动跳过无权限的进程并在诊断信息中显示。

3. **Beta 版本**: 这是首个公开测试版本，如遇到异常或兼容性问题，欢迎提交 Issue。

4. **性能**: 首次文件句柄扫描可能需要几秒钟，取决于系统进程数量和句柄数量。
